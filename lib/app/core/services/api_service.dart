...
//     developer.log('Base URL: $baseUrl', name: 'ApiService');
...
// In ApiService class
  ...
  static ApiResponse<T> _handleResponse<T>(
    ...
    try {
      ...
      if (statusCode >= 200 && statusCode < 300) {
        ...
        // Pass statusCode to _extractErrorMessage
        final errorMessage = _extractErrorMessage(jsonResponse, statusCode);
        _logError('RESPONSE', response.request?.url, errorMessage,
            statusCode: statusCode);
        return ApiResponse<T>(
          success: false,
  ...
  static String _extractErrorMessage(dynamic jsonResponse, int statusCode) {
    String technicalMessage;

    // Debug what type jsonResponse actually is
    print('🔍 jsonResponse type: ${jsonResponse.runtimeType}');
    print('🔍 jsonResponse value: $jsonResponse');
      ...
      for (final field in possibleFields) {
        if (jsonResponse.containsKey(field)) {
          ...
          if (value is String) {
            ...
            technicalMessage = value.join(', ');
          } else if (value is Map) {
            technicalMessage = json.encode(value);
          } else {
            technicalMessage = value.toString();
          }

          return ErrorMessageHelper.getUserFriendlyMessage(
              '$statusCode: $technicalMessage'
          );
        }
      }
      technicalMessage = json.encode(jsonResponse);
    } else if (jsonResponse is String) {
      technicalMessage = jsonResponse;
    } else if (jsonResponse is List) {
      // Handle array responses
      technicalMessage = jsonResponse.join(', ');
    } else {
      // Fallback - convert to string
      technicalMessage = jsonResponse?.toString() ?? 'Unknown error occurred';
    }
  ...
  static ApiResponse<T> _handleError<T>(
    ...
    final userFriendlyMessage = ErrorMessageHelper.getUserFriendlyMessage(message);

    return ApiResponse<T>(
      success: false,
      errorMessage: userFriendlyMessage, // Changed from 'message' to 'userFriendlyMessage'
      statusCode: -1,
    );
  }
